...
class UserForm extends CFormModel
{
    ...
    {
        return array(
            array('username, disp_name, city, in_firm','required'),
    ...
    public function validateInFirm($attribute, $params){
        $firmList = $this->in_firm;
        if(!empty($firmList)){
            if (is_array($firmList)){
                foreach ($firmList as $firm){
                    $rows = Yii::app()->db->createCommand()->select("id")->from("sev_firm")
                        ->where('id=:id',array(':id'=>$firm))->queryRow();
                    if(!$rows){
                        $message = "公司不存在";
    ...
    public function getTemplateData($id) {
        ...
        $sql = "select system_id, a_read_only, a_read_write, a_control from sec_template
				where temp_id=$id 
			";
    ...
    public function retrieveData($index) {
        $suffix = Yii::app()->params['envSuffix'];
        $sql = "select a.* from sec_user a where a.username='$index' and a.username<>'admin'";
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                ...
                $this->city = $row['city'];
                $this->in_firm = explode(",",$row['in_firm']);
    ...
    protected function saveUser(&$connection)
    {
        ...
        switch ($this->scenario) {
            case 'delete':
                $sql = "delete from sec_user where username = :username";
                break;
            case 'new':
                $sql = "insert into sec_user
							(username, password, disp_name, in_firm, email, status, lcu, luu, city)
						values 
							(:username, :password, :dispname, :in_firm, :email, :status, :lcu, :luu, :city)
					";
                break;
            case 'edit':
                $sql = "update sec_user set ";
                if ($hashPass !== '') $sql .= "password = :password, ";
                $sql .= "disp_name = :dispname, email = :email, city = :city, in_firm = :in_firm, "
        ...
        if (strpos($sql,':in_firm')!==false){
            $firm = implode(",",$this->in_firm);
            $command->bindParam(':in_firm',$firm,PDO::PARAM_STR);
        }
    ...
    protected function saveRights(&$connection) {
        ...
        switch ($this->scenario) {
            case 'delete':
                $sql = "delete from sec_user_access where username = :username and system_id :system_id";
                break;
            case 'new':
            case 'edit':
                $sql = "insert into sec_user_access 
    ...
    protected function saveInfo(&$connection) {
        ...
        switch ($this->scenario) {
            case 'delete':
                $sql = "delete from sec_user_info where username = :username";
                break;
            case 'new':
            case 'edit':
                $sql = "insert into sec_user_info 
