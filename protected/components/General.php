...
class General {
    ...
    public static function getCityList()
    {
        ...
        $sql = "select code, name from sec_city order by name";
        $rows = Yii::app()->db->createCommand($sql)->queryAll();
        if (count($rows) > 0) {
            foreach ($rows as $row) {
    ...
    public static function getCityListWithNoDescendant($city_allow='') {
        ...
        $sql = "select distinct a.code, a.name from sec_city a 
					left outer join sec_city b on a.code=b.region 
    ...
    public static function getMgrFeedbackList()
    {
        ...
        $sql = "select a.username, a.disp_name, a.email 
				from sec_user a, sec_user_access b
    ...
    public static function getEmailByUserId($uid) {
        $suffix = Yii::app()->params['envSuffix'];
        $sql = "select email from sec_user where username='".$uid."'";
    ...
    public static function getCityName($code) {
        $suffix = Yii::app()->params['envSuffix'];
        $sql = "select name from sec_city where code='$code'";
